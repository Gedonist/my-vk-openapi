...
- `GET /vk/logs` — просмотр сохраненных callback-событий, защищен `API_AUTH_TOKEN`.
- Swagger UI доступен на `/`.
- OpenAPI JSON доступен на `/openapi.json`.
- В Swagger UI добавлена авторизация для защищенных методов:
  - `BearerAuth` — вставьте `API_AUTH_TOKEN`, Swagger сам отправит `Authorization: Bearer ...`;
...
При каждом изменении поведения API увеличивайте версию в двух местах:
...
- `BearerAuth`: вставьте значение `API_AUTH_TOKEN` без слова `Bearer`;
- `ApiTokenHeader`: вставьте значение `API_AUTH_TOKEN`.
